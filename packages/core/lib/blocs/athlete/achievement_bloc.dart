...
sealed class AchievementEvent with _$AchievementEvent {
  const factory AchievementEvent.createAchievement(Achievement achievement) = _CreateAchievement;
  const factory AchievementEvent.getAchievementById(String id) = _GetAchievementById;
  const factory AchievementEvent.getAchievementByUserId(String userId) = _GetAchievementByUserId;
  const factory AchievementEvent.getAllAchievements({
    @Default(1) int page,
    @Default(10) int limit,
  }) = _GetAllAchievements;
  const factory AchievementEvent.updateAchievement(String id, Achievement achievement) = _UpdateAchievement;
  const factory AchievementEvent.deleteAchievement(String id) = _DeleteAchievement;
}
...
sealed class AchievementState with _$AchievementState {
  ...
  const factory AchievementState.loadedAchievement(Achievement achievement) = LoadedAchievement;
...
class AchievementBloc extends Bloc<AchievementEvent, AchievementState> {
  ...
  Future<void> _onCreateAchievement(
    ...
    try {
      final createdAchievement = await achievementRepository.createAchievement(event.achievement);
  ...
  Future<void> _onGetAchievementById(
    ...
    emit(const AchievementState.loading());
    try {
      final achievement = await achievementRepository.getAchievementById(event.id);
  ...
  Future<void> _onGetAchievementByUserId(
    ...
    emit(const AchievementState.loading());
    try {
      final achievement = await achievementRepository.getAchievementByUserId(event.userId);
      emit(AchievementState.loadedAchievement(achievement));
    } catch (e) {
      emit(AchievementState.error(e.toString()));
    }
