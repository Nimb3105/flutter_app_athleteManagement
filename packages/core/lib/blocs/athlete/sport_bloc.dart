...
sealed class SportEvent with _$SportEvent {
  ...
  const factory SportEvent.getAllSports() = GetAllSports;
...
sealed class SportState with _$SportState {
  ...
  const factory SportState.loadedSports(List<Sport> sports) = LoadedSports;
...
class SportBloc extends Bloc<SportEvent, SportState> {
  ...
  Future<void> _onCreateSport(
      CreateSport event, Emitter<SportState> emit) async {
  ...
  Future<void> _onGetSportById(
      GetSportById event, Emitter<SportState> emit) async {
    emit(const SportState.loading());
    try {
  ...
  Future<void> _onGetAllSports(
      GetAllSports event, Emitter<SportState> emit) async {
    emit(const SportState.loading());
    try {
      final sports = await sportRepository.getAllSports();
      emit(SportState.loadedSports(sports));
    } catch (e) {
      emit(SportState.error(e.toString()));
    }
  }
  ...
  Future<void> _onUpdateSport(
      UpdateSport event, Emitter<SportState> emit) async {
    emit(const SportState.loading());
    try {
      final updatedSport =
          await sportRepository.updateSport(event.id, event.sport);
  ...
  Future<void> _onDeleteSport(
      DeleteSport event, Emitter<SportState> emit) async {
    emit(const SportState.loading());
    try {
