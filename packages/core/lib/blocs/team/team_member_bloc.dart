...
sealed class TeamMemberEvent with _$TeamMemberEvent {
  const factory TeamMemberEvent.createTeamMember(TeamMember teamMember) = _CreateTeamMember;
  const factory TeamMemberEvent.getTeamMemberById(String id) = _GetTeamMemberById;
  const factory TeamMemberEvent.getTeamMembersByTeamId(String teamId) = _GetTeamMembersByTeamId;
  const factory TeamMemberEvent.getTeamMembersByUserId(String userId) = _GetTeamMembersByUserId;
  const factory TeamMemberEvent.getAllTeamMembers({
    @Default(1) int page,
    @Default(10) int limit,
  }) = _GetAllTeamMembers;
  const factory TeamMemberEvent.updateTeamMember(String id, TeamMember teamMember) = _UpdateTeamMember;
...
sealed class TeamMemberState with _$TeamMemberState {
  ...
  const factory TeamMemberState.loadedTeamMember(TeamMember teamMember) = LoadedTeamMember;
...
class TeamMemberBloc extends Bloc<TeamMemberEvent, TeamMemberState> {
  ...
  TeamMemberBloc({required this.teamMemberRepository})
  ...
  Future<void> _onCreateTeamMember(
    ...
    try {
      final createdTeamMember = await teamMemberRepository.createTeamMember(event.teamMember);
  ...
  Future<void> _onGetTeamMembersByTeamId(
    ...
    emit(const TeamMemberState.loading());
    try {
      final teamMembers = await teamMemberRepository.getTeamMembersByTeamId(event.teamId);
      emit(TeamMemberState.loadedTeamMembers(teamMembers, 1, teamMembers.length, false));
    } catch (e) {
      emit(TeamMemberState.error(e.toString()));
    }
  }
  ...
  Future<void> _onGetTeamMembersByUserId(
    ...
    emit(const TeamMemberState.loading());
    try {
      final teamMembers = await teamMemberRepository.getTeamMembersByUserId(event.userId);
      emit(TeamMemberState.loadedTeamMembers(teamMembers, 1, teamMembers.length, false));
    } catch (e) {
      emit(TeamMemberState.error(e.toString()));
    }
    ...
    try {
      ...
      emit(
        TeamMemberState.loadedTeamMembers(
          result['teamMembers'] as List<TeamMember>,
          event.page,
          event.limit,
