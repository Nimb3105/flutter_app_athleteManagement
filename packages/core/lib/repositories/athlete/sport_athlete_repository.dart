...
class SportAthleteRepository {
  final String baseUrl;

  SportAthleteRepository({required this.baseUrl});

  // Create a new sport athlete
  Future<SportAthlete> createSportAthlete(SportAthlete sportAthlete) async {
    final response = await http.post(
      Uri.parse('$baseUrl/sport-athletes'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode(sportAthlete.toJson()),
    );

    if (response.statusCode == 200 || response.statusCode == 201) {
      final data = jsonDecode(response.body) as Map<String, dynamic>;
      if (data['data'] != null && data['data'] is Map<String, dynamic>) {
        return SportAthlete.fromJson(data['data'] as Map<String, dynamic>);
      } else {
  ...
  Future<SportAthlete> getSportAthleteById(String id) async {
    final response = await http.get(
      Uri.parse('$baseUrl/sport-athletes/$id'),
      headers: {'Content-Type': 'application/json'},
    );
  ...
  Future<SportAthlete> getSportAthleteByUserId(String userId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/sport-athletes/user/$userId'),
      headers: {'Content-Type': 'application/json'},
    );
  ...
  Future<List<SportAthlete>> getSportAthleteBySportId(String sportId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/sport-athletes/sport/$sportId'),
      headers: {'Content-Type': 'application/json'},
    );
    ...
    if (response.statusCode == 200) {
      ...
      if (data['data'] != null && data['data'] is List<dynamic>) {
        ...
        return jsonList
            .map((json) => SportAthlete.fromJson(json as Map<String, dynamic>))
            .toList();
      } else {
  ...
  Future<List<SportAthlete>> getAllSportAthletes() async {
    final response = await http.get(
      Uri.parse('$baseUrl/sport-athletes'),
      headers: {'Content-Type': 'application/json'},
    );
  ...
  Future<SportAthlete> updateSportAthlete(
    String id,
  ...
  Future<void> deleteSportAthlete(String id) async {
...
// Get all sport athletes by user ID
  // Future<List<SportAthlete>> getAllSportAthletesByUserId(String userId) async {
  //   final response = await http.get(
  //     Uri.parse('$baseUrl/sport-athletes/user/$userId/all'),
  //     headers: {'Content-Type': 'application/json'},
  //   );
    ...
    if (response.statusCode == 200) {
      ...
      return jsonList.map((json) {
        if (json is Map<String, dynamic>) {
          return SportAthlete.fromJson(json);
        } else {
